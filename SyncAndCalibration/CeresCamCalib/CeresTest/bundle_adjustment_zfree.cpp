...
class BundleAdjustmentProblemZFree {
public:
	struct ReprojectionError {
		...
		static ceres::CostFunction* Create(Configs *cf, double *observed_points) {
			...
			ReprojectionError *re = new BundleAdjustmentProblemZFree::ReprojectionError(cf, 88, observed_points);
...
double BundleAdjustmentProblemZFree::ReprojectionError::chb_points[NUM_CORNERS * 3] = { -0, 0, 0,
-60, 0, 0,
-120, 0, 0,
-180, 0, 0,
-240, 0, 0,
...
BundleAdjustmentZFree::BundleAdjustmentZFree() {
}
int BundleAdjustmentZFree::DoWork(std::string work_path) {
	google::InitGoogleLogging("HJP");

	// load input
	BundleAdjustmentProblemZFree prob;
	...
	const char* output_configs_path = "C:/Users/joont/Documents/PycharmProjects/CameraCalibration/data/BundleAdjustment/output/bundle_adjustment_zfree/configs.txt";
	configs->ExportConfigs(output_configs_path);

	// output
	std::ofstream output_txt;
	output_txt.open("C:/Users/joont/Documents/PycharmProjects/CameraCalibration/data/BundleAdjustment/output/bundle_adjustment_zfree/bundleadjustment_output.txt");
	...
	for (int frame_idx = 0; frame_idx < num_frames; frame_idx++) {
		...
		if (num_detected >= 2) {
			for (int cam_idx = 0; cam_idx < num_cams; cam_idx++) {
				if (prob.detected(frame_idx, cam_idx)) {
					ceres::CostFunction* cost_func = BundleAdjustmentProblemZFree::ReprojectionError::Create(prob.GetConfigs(), prob.image_points(frame_idx, cam_idx));
					double *chb_rvec = prob.chb_rvec(frame_idx);
					double *chb_tvec = prob.chb_tvec(frame_idx);
					double *cam_params = prob.cam_params(cam_idx);

					ceres_prob.AddResidualBlock(cost_func, loss, cam_params, chb_rvec, chb_tvec, chb_points_z);
				}
			}
		}
	}
	...
	double *chb_points = BundleAdjustmentProblemZFree::ReprojectionError::chb_points;
	...
	for (int frame_idx = 0; frame_idx < configs->num_frames; frame_idx++) {
		...
		for (int p_idx = 0; p_idx < configs->chb_num_corners; p_idx++) {
			double chb_point[3] = { chb_points[p_idx * 3], chb_points[p_idx * 3 + 1], chb_points_z[p_idx] };
	...
	for (int p_idx = 0; p_idx < configs->chb_num_corners; p_idx++) {
		output_txt << chb_points[p_idx * 3] << " " << chb_points[p_idx * 3 + 1] << " " << chb_points_z[p_idx] << " ";
	}
	output_txt << "\n";
