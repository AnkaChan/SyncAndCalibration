...
class BundleAdjustmentProblem6DofUp {
public:
	struct ReprojectionError {
		...
		static ceres::CostFunction* Create(Configs *cf, double *observed_points) {
			...
			ReprojectionError *re = new BundleAdjustmentProblem6DofUp::ReprojectionError(cf, 88, observed_points);
			return (new ceres::AutoDiffCostFunction<ReprojectionError, (88 * 2) + (6 + 2) + (2 * (10 * 7) + 10 + 7), kNumCamParam, kChbRvecDim, kChbTvecDim, 88 * 3>(re));
		}

		template<typename T>
		bool operator()(const T* const camera, const T* const chbR, const T* const chbT, const T* const chb_points, T* residuals) const {
			...
			idx += 8;
			for (int test = 0; test < 2 * (10 * 7) + (10 + 7); test++) {
				residuals[idx + test] = T(0);
			}
			...
			for (int r = 0; r < 7; r++) {
				for (int c = 0; c < 10; c++) {
					int i = (r * 11 + c) * 3;
					T p0[3] = { chb_points[i + 0], chb_points[i + 1], chb_points[i + 2] };

					int i1 = (r * 11 + c + 1) * 3;
					T p1[3] = { chb_points[i1 + 0], chb_points[i1 + 1], chb_points[i1 + 2] };

					int i2 = ((r + 1) * 11 + c) * 3;
					T p2[3] = { chb_points[i2 + 0], chb_points[i2 + 1], chb_points[i2 + 2] };

					T p1p0[3] = { p1[0] - p0[0], p1[1] - p0[1], p1[2] - p0[2] };
					T p2p0[3] = { p2[0] - p0[0], p2[1] - p0[1], p2[2] - p0[2] };

					T L10 = sqrt(p1p0[0] * p1p0[0] + p1p0[1] * p1p0[1] + p1p0[2] * p1p0[2]);
					T L20 = sqrt(p2p0[0] * p2p0[0] + p2p0[1] * p2p0[1] + p2p0[2] * p2p0[2]);
					residuals[idx + ri + 0] = T(0)*T(S - L10);
					residuals[idx + ri + 1] = T(0)*T(S - L20);
					ri += 2;
				}
			}
			for (int r = 0; r < 7; r++) {
...
BundleAdjustment6DofUp::BundleAdjustment6DofUp() {
}
int BundleAdjustment6DofUp::DoWork(std::string work_path) {
	google::InitGoogleLogging("HJP");

	// load input
	BundleAdjustmentProblem6DofUp prob;
	...
	//prob.GenerateInitialWorldPoints();
	configs->max_k = 6;
	configs->max_p = 2;
	...
	const char* output_configs_path = "C:/Users/joont/Documents/PycharmProjects/CameraCalibration/data/BundleAdjustment/output/bundle_adjustment_6dof+/configs.txt";
	configs->ExportConfigs(output_configs_path);

	// output
	std::ofstream output_txt;
	output_txt.open("C:/Users/joont/Documents/PycharmProjects/CameraCalibration/data/BundleAdjustment/output/bundle_adjustment_6dof+/bundleadjustment_output.txt");
	...
	double *chb_points = prob.chb_points();
	for (int frame_idx = 0; frame_idx < num_frames; frame_idx++) {
		...
		if (num_detected >= 2) {
			for (int cam_idx = 0; cam_idx < num_cams; cam_idx++) {
				if (prob.detected(frame_idx, cam_idx)) {
					ceres::CostFunction* cost_func = BundleAdjustmentProblem6DofUp::ReprojectionError::Create(prob.GetConfigs(), prob.image_points(frame_idx, cam_idx));
					double *chb_rvec = prob.chb_rvec(frame_idx);
					double *chb_tvec = prob.chb_tvec(frame_idx);
					double *cam_params = prob.cam_params(cam_idx);

					ceres_prob.AddResidualBlock(cost_func, loss, cam_params, chb_rvec, chb_tvec, chb_points);
				}
			}
		}
	}
	...
	for (int frame_idx = 0; frame_idx < configs->num_frames; frame_idx++) {
		...
		for (int p_idx = 0; p_idx < configs->chb_num_corners; p_idx++) {
			double chb_point[3] = { chb_points[p_idx * 3], chb_points[p_idx * 3 + 1], chb_points[p_idx * 3 + 2] };
	...
	for (int p_idx = 0; p_idx < configs->chb_num_corners; p_idx++) {
		output_txt << chb_points[p_idx * 3] << " " << chb_points[p_idx * 3 + 1] << " " << chb_points[p_idx * 3 + 2] << " ";
	}
	output_txt << "\n";
