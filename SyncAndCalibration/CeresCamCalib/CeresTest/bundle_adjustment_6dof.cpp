...
public:
	struct ReprojectionError {
		...
		static ceres::CostFunction* Create(Configs *cf, double *observed_points) {
			...
			ReprojectionError *re = new BundleAdjustmentProblem6Dof::ReprojectionError(cf, 88, observed_points);
			return (new ceres::AutoDiffCostFunction<ReprojectionError, (88 * 2) + 6 + 2, kNumCamParam, kChbRvecDim, kChbTvecDim>(re));
		}
...
BundleAdjustment6Dof::BundleAdjustment6Dof() {
}
int BundleAdjustment6Dof::DoWork(std::string work_path) {
	...
	configs->input_initial_params_path = work_path + "/BundleAdjustment/input/bund_adj_inital_params.json";
	...
	const char* output_configs_path = (work_path + std::string("/BundleAdjustment/output/bundle_adjustment_6dof/configs.txt")).c_str();
	configs->ExportConfigs(output_configs_path);

	// output
	std::ofstream output_txt;
	output_txt.open(work_path + "/BundleAdjustment/output/bundle_adjustment_6dof/bundleadjustment_output.txt");
	...
	for (int frame_idx = 0; frame_idx < num_frames; frame_idx++) {
		...
		if (num_detected >= 2) {
			for (int cam_idx = 0; cam_idx < num_cams; cam_idx++) {
				if (prob.detected(frame_idx, cam_idx)) {
					ceres::CostFunction* cost_func = BundleAdjustmentProblem6Dof::ReprojectionError::Create(prob.GetConfigs(), prob.image_points(frame_idx, cam_idx));
